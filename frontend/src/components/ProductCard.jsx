...
export default function ProductCard({ product, onOpenDetails }) {
  ...
  // Sécurisation et normalisation des données numériques et textuelles
  ...
  const isAvailable = Boolean(
    product?.isAvailable ?? Number(product?.stock ?? 0) > 0,
  );
    ...
    >
      ...
      <div className="h-44 bg-gray-100 relative">
        ...
        <div className="absolute top-3 right-3">
          <span
            className={`text-[10px] font-bold px-2.5 py-1 rounded-full text-white shadow-sm uppercase tracking-wider ${
              isAvailable ? "bg-brand-green" : "bg-red-500"
            }`}
          >
      ...
      <div className="p-4 flex flex-col flex-grow justify-between">
        ...
        <div className="flex items-end justify-between mt-4 border-t border-gray-100 pt-3">
          ...
          <div className="text-right">
            <p className="text-[9px] text-brand-green font-bold uppercase">
              Prix TTC ({vatRate}%)
            </p>
