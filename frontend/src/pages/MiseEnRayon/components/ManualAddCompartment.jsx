// src/pages/MiseEnRayon/components/ManualAddCompartment.jsx
...
export default function ManualAddCompartment({ onProductAdded }) {
  ...
  const producerDept = user?.department || user?.departmentCode || user?.city || "Dépt. Local";
  const producerLocation = user?.address || user?.city || user?.companyName || "Exploitation locale";
  const producerName = user?.companyName || user?.displayName || user?.name || user?.producerName || "Maraîcher Exploitant";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Appel unique au Service Métier (Séparation des responsabilités)
      ...
      // Réinitialisation intégrale du formulaire
      setFormData({
        title: "",
        category: "Légumes",
        priceHT: "",
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-6">
      <div className="flex items-center justify-between border-b border-gray-100 pb-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            Saisissez les informations de votre récolte. L'origine ({producerDept}) et le lieu sont hérités de votre profil.
          </p>
        </div>
      ...
      {statusMsg.text && (
        <div className={`p-3.5 rounded-xl text-xs font-bold flex items-center gap-2 animate-fade-in ${statusMsg.type === 'error' ? 'bg-red-50 border-red-200 text-red-800' : 'bg-green-50 border-green-200 text-green-800'}`}>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div className="lg:col-span-2">
            <label className="text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5 flex">Désignation du Produit *</label>
            <input
              type="text"
              required
          ...
          <div>
            <label className="text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5 flex">Catégorie *</label>
            <select
          ...
          <div>
            <label className="text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5 flex">Prix Unitaire HT (€) *</label>
            <input
              type="number"
              step="0.01"
              min="0.01"
          ...
          <div>
            <label className="text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5 flex">Taux de TVA *</label>
            <select
              className="w-full border border-gray-300 rounded-xl px-3.5 py-2.5 text-xs font-semibold text-gray-800 bg-white focus:ring-2 focus:ring-emerald-500"
          ...
          <div>
            <label className="text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5 flex">Unité de Vente *</label>
            <select
              className="w-full border border-gray-300 rounded-xl px-3.5 py-2.5 text-xs font-semibold text-gray-800 bg-white focus:ring-2 focus:ring-emerald-500"
              value={formData.unit}
          ...
          <div>
            <label className="text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5 flex">Quantité en Stock *</label>
            <input
              type="number"
              min="0"
              required
          ...
          <div>
            <label className="text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5 flex">Date de Récolte *</label>
            <input
              type="date"
              required
          ...
          <div>
            <label className="text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5 flex">N° de Lot / Traçabilité</label>
            <input
              type="text"
        ...
        <div>
          <label className="text-xs font-bold text-gray-700 uppercase tracking-wider mb-2 flex">Photo du Produit</label>
        ...
        <div className="flex items-center">
          <label className="flex items-start gap-3 p-3.5 border border-amber-200 bg-amber-50/40 rounded-xl cursor-pointer hover:bg-amber-50 transition w-full">
            ...
            />
            <div>
              <span className="text-xs font-extrabold text-amber-900 uppercase tracking-wider flex">Label Agriculture Biologique (AB / Bio)</span>
