...
export default function ProductDetailPage({
  ...
  // 🚀 Action lors du clic : Ouvre la vitrine dédiée du producteur
  const handleProducerClick = () => {
    if (typeof onOpenProducerStore === "function") {
      onOpenProducerStore(producerId, producerName);
    }
  };

  const otherProducerProducts = allProducts.filter((p) => {
    const pProducer = p.producerCompany || p.producerName || p.producer || p.companyName;
    const isSameProducer =
      (p.producerId && p.producerId === producerId) ||
      ...
      <div className="bg-white border border-gray-200 rounded-3xl p-6 shadow-sm grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
        ...
        <div className="md:col-span-7 space-y-5">
          <div>
            ...
            {/* 🔗 BOUTON NOM DU FOURNISSEUR INTERACTIF */}
            <button
              type="button"
              onClick={handleProducerClick}
              className="group text-xs text-emerald-700 hover:text-emerald-800 font-extrabold uppercase tracking-wider mt-1.5 flex items-center gap-1.5 cursor-pointer bg-emerald-50 hover:bg-emerald-100 px-3 py-1.5 rounded-xl border border-emerald-200 transition-all w-fit focus:outline-none"
              title={`Voir la page vitrine et le catalogue de ${producerName}`}
            >
              <Store size={14} className="shrink-0 text-emerald-600 group-hover:scale-110 transition-transform" />
              <span className="underline decoration-emerald-400 underline-offset-2">{producerName}</span>
              <span className="text-[10px] text-emerald-800 font-normal ml-1">(Voir la page producteur)</span>
            </button>
          </div>
