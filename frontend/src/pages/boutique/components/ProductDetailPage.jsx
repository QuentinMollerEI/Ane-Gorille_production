 ...
 * Fiche produit complète avec traçabilité maraîchère.
 * Filtre strictement les autres produits masqués du même producteur.
 */
export default function ProductDetailPage({
  ...
  const producerAddress =
    product?.producerAddress || "Adresse validée au registre";
  const producerCity = product?.producerCity || "Commune locale";
  const producerZipCode = product?.producerZipCode || "";
  const producerDepartment =
    product?.producerDepartment || producerZipCode.substring(0, 2) || "31";

  // Filtre strict : Même producteur, produit différent, ET NON MASQUÉ
  const otherProducerProducts = allProducts.filter((p) => {
    const isSameProducer =
      p.producerId === product.producerId ||
      p.producerCompany === product.producerCompany;
    ...
    <div className="space-y-6 animate-fade-in max-w-5xl mx-auto pb-12 text-xs">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-[11px] font-bold text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
          Fiche de Traçabilité N° {product.id?.substring(0, 8) || "REF-AGRI"}
        </span>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-3xl p-6 shadow-sm grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
        ...
        {/* INFORMATIONS & DÉTAILS DU PRODUIT */}
        <div className="md:col-span-7 space-y-5">
          <div>
            ...
            <p className="text-xs text-gray-500 font-extrabold uppercase tracking-wider mt-0.5">
              🏡 {producerName}
            </p>
          </div>
          ...
          <div className="p-4 bg-emerald-50/60 border border-emerald-200 rounded-2xl space-y-1">
            <p className="font-bold text-gray-800 flex items-center gap-1.5">
              ...
              <span>
                {producerAddress} {producerZipCode} {producerCity}
              </span>
            </p>
            <p className="text-[10px] text-gray-500 italic">
              🔒 Adresse certifiée au registre des exploitants agricoles.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3.5 bg-gray-50 border border-gray-200 rounded-2xl">
              <span className="text-gray-400 font-extrabold uppercase text-[9px] block">
                Tarif Unitaire HT
              </span>
              <p className="text-base font-black text-gray-900">
                {priceHT.toFixed(2)} € / {product.unit || "kg"}
              </p>
              <p className="text-emerald-800 font-bold text-[10px]">
            ...
            <div className="space-y-2 pt-2">
              <div className="flex items-center gap-3">
                ...
                >
                  <ShoppingBag size={16} />
                  <span>
                    Ajouter au panier ({(priceTTC * quantity).toFixed(2)} € TTC)
                  </span>
                </button>
              </div>
            </div>
      ...
      {/* SECTION : AUTRES PRODUITS DE CETTE FERME (EXCLUSION STRICTE DES MASQUÉS) */}
      {otherProducerProducts.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-3xl p-6 shadow-sm space-y-4">
          <h3 className="font-extrabold text-gray-900 text-sm flex items-center gap-2">
            <Building size={16} className="text-emerald-700" />
            <span>
              Toutes les autres cultures disponibles de {producerName}
            </span>
          </h3>
