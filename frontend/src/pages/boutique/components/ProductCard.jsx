...
export default function ProductCard({ product, onSelectProduct, onAddToCart }) {
  ...
  const producerName = (product?.producerCompany || product?.producerName || "EXPLOITATION LOCALE").toUpperCase();
  // Utilisation exclusive du département calculé depuis le code postal
  const deptCode = product?.producerDepartment || "31";
  ...
  return (
    ...
    >
      <div className="absolute top-5 right-5 z-10">
        {isAvailable ? (
          <span className="bg-emerald-600 text-white font-black text-[10px] px-2.5 py-0.5 rounded-full uppercase tracking-wider shadow-sm">En Stock</span>
        ) : (
          <span className="bg-red-600 text-white font-black text-[10px] px-2.5 py-0.5 rounded-full uppercase tracking-wider shadow-sm">Épuisé</span>
        )}
      </div>
      ...
      <div className="relative w-full h-36 rounded-xl overflow-hidden bg-gray-100 flex items-center justify-center border border-gray-150">
        {imageSrc ? (
          ...
          <div className="flex flex-col items-center justify-center text-gray-400 space-y-1">
            <Camera size={26} />
            <span className="text-[10px] font-bold">Aucun visuel</span>
          </div>
        )}
      ...
      <div className="space-y-1">
        ...
        <h3 className="font-black text-gray-900 text-sm group-hover:text-emerald-800 transition-colors line-clamp-1">{product.title || product.name}</h3>
      </div>

      <div className="pt-2 border-t border-gray-100 flex items-end justify-between gap-2">
        <div>
          <span className="text-[9px] font-black text-gray-400 uppercase block">Prix HT</span>
          <p className="text-sm font-black text-gray-900 leading-tight">{priceHT.toFixed(2)} €</p>
        </div>
        <div className="text-right">
          <span className="text-[9px] font-black text-emerald-800 uppercase block">Prix TTC ({vatRate}%)</span>
          <p className="text-xs font-black text-emerald-800 leading-tight">{priceTTC.toFixed(2)} € / {product.unit || "kg"}</p>
        </div>
      </div>
      ...
      <div className="bg-gray-50 border border-gray-200/80 rounded-xl p-2 flex items-center justify-between text-[11px]">
        <div className="flex items-center gap-1 font-bold text-gray-600">
          ...
          <span className="font-extrabold text-amber-900">{stock} {product.unit || "kg"}</span>
        </div>
        {isAvailable && onAddToCart && (
          <button
            type="button"
            onClick={(e) => { e.stopPropagation(); onAddToCart(product, 1); }}
            className="p-1.5 bg-emerald-700 hover:bg-emerald-800 text-white rounded-lg transition-colors flex items-center gap-1 font-bold text-[10px] cursor-pointer shadow-xs z-20"
          >
            <ShoppingBag size={12} /><span>+ Panier</span>
          </button>
        )}
      </div>
    </div>
