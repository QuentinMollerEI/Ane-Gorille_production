...
/**
 * COMPORTEMENT DE TÉLÉCHARGEMENT DIRECT
 * Génère dynamiquement un justificatif d'achat au format texte .txt
 */
const downloadDocumentAsFile = (docData) => {
  const border = "=========================================================";
  const content = `
${border}
               JUSTIFICATIF COMPTABLE - ÂNE & GORILLE
${border}
N° de Pièce : ${docData.id}
Réf. Commande : ${docData.orderId || "-"}
Émis le : ${docData.date || "N/A"}
Émetteur : ${docData.entity || "Plateforme Âne & Gorille (Vente)"}

ACHETEUR :
----------
Nom/Établissement : ${docData.buyerName || "Client local"}
Réf. Engagement : ${docData.refEngagement || "-"}

DÉTAIL DE LA TRANSACTION :
--------------------------
Type de Pièce : ${docData.type}
Montant de la Pièce : ${docData.amount?.toFixed(2)} € (TTC)
Taux de TVA Appliqué : ${docData.vatRate || 5.5}%

MENTION LÉGALE :
----------------
"TVA non applicable, art. 293 B du Code Général des Impôts (CGI)."
Mandat administratif de paiement à 30 jours (Loi LME - pour les organismes publics).

${border}
Document archivé et certifié conforme par la plateforme de distribution locale.
  `;
...
function BuyerIndicators({ documents }) {
  ...
  const averageBasket =
    totalSpent > 0
      ? totalSpent / documents.filter((doc) => doc.status === "paid").length
      : 0;
      ...
      {!isRetracted && (
        <div className="p-5 space-y-4 animate-fade-in">
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <select
              className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-1 focus:ring-green-500 focus:border-green-500 text-gray-600 bg-white"
              value={docFilter}
...
// =========================================================================
// COMPOSANT PARENT : PiecesComptables (Acheteur Container unifié)
// =========================================================================
export default function PiecesComptables() {
  ...
  // 📡 ÉCOUTEUR FIRESTORE TEMPS RÉEL SUR LA COLLECTION UNIFIÉE
  useEffect(() => {
    if (!user?.uid) {
      setLoading(false);
      return;
    ...
    const unsubscribe = onSnapshot(
      ...
      (err) => {
        ...
        setError(
          "Erreur de synchronisation ou de permissions avec Firestore Database.",
        );
        setLoading(false);
      },
    );
