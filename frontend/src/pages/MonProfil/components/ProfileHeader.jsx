...
import { ShieldCheck, AlertTriangle, Building2, UserCircle } from "lucide-react";

/**
 * COMPOSANT : ProfileHeader.jsx
 * Affiche l'en-tête de l'espace profil et le badge de conformité 100%.
 * Ce badge est l'indicateur visuel de déblocage de la plateforme.
 */
export default function ProfileHeader({ roleLabel, isComplete, icon: Icon = Building2 }) {
  return (
      ...
      <div className="space-y-2">
        ...
        {isComplete ? (
          <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-xl font-extrabold text-xs">
            ...
            <span>Profil vérifié & conforme à 100% (Accès complet autorisé)</span>
          </div>
        ) : (
          <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-amber-50 border border-amber-200 text-amber-800 rounded-xl font-extrabold text-xs animate-pulse">
            ...
            <span>Informations Incomplètes : Votre compte est restreint au mode "Lecture seule"</span>
          </div>
        )}
      </div>
      
      <div className="p-3 bg-slate-100 text-slate-700 rounded-2xl self-start sm:self-auto border border-slate-200 hidden sm:block">
        <Icon size={28} />
      </div>
    </div>
  );
