 ...
 * 
 * Fonctionnalités :
 * 1. Consultation des colis conditionnés en caisses consignées réutilisables.
 * 2. Visualisation du N° de lot sanitaire HACCP attribué.
 * 3. Modale d'impression officielle du Bon de Récolte & Préparation (BP) conforme.
 */
export default function ReadyToShipCompartment({ subOrders = [] }) {
  ...
  // Filtrage dynamique par recherche (Nom de l'acheteur ou N° de lot)
  ...
  return (
    <div className="space-y-4 text-xs">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-3.5 shadow-xs flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
      ...
      {filteredSubs.length > 0 ? (
        <div className="space-y-3">
          {filteredSubs.map((sub) => {
            ...
            const isDelivered = sub.status === "DELIVERED" || sub.status === "TERMINE";
            const isShipped = sub.status === "EXPEDIE" || sub.status === "EN_COURS_DE_LIVRAISON";
            ...
            return (
              <div
                key={sub.id}
                className="bg-white border border-gray-200 rounded-lg p-3.5 space-y-3 shadow-2xs hover:border-gray-300 transition-colors"
              >
                {/* En-tête du colis */}
                ...
                <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                  ...
                  <button
                    ...
                    className="px-3.5 py-1.5 bg-gray-900 hover:bg-black text-white rounded-md text-xs font-bold flex items-center gap-1.5 transition-all shadow-xs cursor-pointer"
                  >
                    <Printer size={13} />
          ...
          <div className="bg-white rounded-lg max-w-xl w-full max-h-[90vh] overflow-y-auto shadow-2xl p-6 space-y-4 text-gray-900">
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-gray-200">
              ...
              <button
                ...
                className="px-5 py-1.5 bg-emerald-700 hover:bg-emerald-800 text-white font-extrabold rounded text-xs flex items-center gap-1.5 cursor-pointer shadow-xs"
              >
                <Printer size={14} />
