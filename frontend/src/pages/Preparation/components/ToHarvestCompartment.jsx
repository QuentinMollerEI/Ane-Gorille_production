...
import { validatePreparation } from "../../../services/CheckoutOrchestrator";
...
export default function ToHarvestCompartment({ subOrders = [], onRefresh }) {
  ...
  // 1. Synthèse globale cumulée par produit pour la cueillette au champ
  ...
  // Action : Valider la préparation avec gestion unifiée Firestore via CheckoutOrchestrator
  const handleValidatePreparation = async (subOrder) => {
    ...
    try {
      if (typeof validatePreparation === "function") {
        await validatePreparation(subOrder, finalLot);
      } else {
  ...
  return (
    <div className="space-y-5 text-xs">
      {/* 📋 1. SYNTHÈSE CUMULÉE DE CUEILLETTE */}
      <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm space-y-3">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-gray-200 pb-2.5 gap-2">
          <div>
              ...
              >
                <div>
                  <div className="flex items-center gap-1.5">
                    ...
                    {item.isBio && (
                      <span className="bg-amber-100 text-amber-900 font-black text-[8px] px-1 py-0.5 rounded uppercase">
                        BIO
                      </span>
                    )}
                  </div>
      ...
      {/* 📦 2. BONS DE PRÉPARATION PAR ACHETEUR */}
      <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm space-y-3">
          ...
          <div className="space-y-3 pt-1">
            {subOrders.map((sub) => {
              ...
              return (
                <div
                  key={sub.id}
                  className="border border-gray-200 rounded-lg p-3.5 bg-white space-y-3 shadow-sm hover:border-gray-300 transition-colors"
                >
                  ...
                  {/* Saisie Lot Sanitaire & Validation */}
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-gray-50 p-2.5 rounded-md border border-gray-200 pt-3">
                    ...
                    <button
                      ...
                      disabled={isLoading}
                      className="w-full sm:w-auto px-4 py-2 bg-emerald-700 hover:bg-emerald-800 text-white font-extrabold rounded-md text-xs uppercase tracking-wider transition-all shadow-sm flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50"
                    >
                      {isLoading ? (
